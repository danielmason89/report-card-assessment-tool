<template>
    <section aria-labelledby="cta" class="container grid gap-4 text-center">
        ...
        <p class="max-w-2xl mx-auto text-muted drop-shadow-text-sm">
        ...
        <form @submit.prevent="handleFormSubmit" id="contact-form"
            ...
            <input v-if="formState !== 'success'" v-show="formState !== 'pending'" type="email" name="email" id="email"
                v-model="email" required placeholder="Email Address"
                class="flex-1 w-full p-2 mx-4 text-sm bg-transparent border-b-2 border-transparent rounded-none caret-accent placeholder:text-black focus:placeholder:text-muted focus:outline-none focus:border-accent">
